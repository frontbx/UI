    ...
    each(menu, (i, item) =>
    {
        if (typeof item === 'string')
        {
            if (!item.includes('iframe'))
            {
                ...
                HTML += `${LB_CHRAR}${TAB_CHAR.repeat(tabIndex)}<li class="menu-item ${active}"><a data-pjax-target="#article-body" href="${back}${slug}" class="js-pjax-link js-docs-menu-link">${item}</a></li>`;
            }
        }
        else
        {
